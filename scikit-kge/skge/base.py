...
class Model(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.params = {}
        self.hyperparams = {}
        self.add_hyperparam('inite', kwargs.pop('inite', 'nunif'))
        self.add_hyperparam('initr', kwargs.pop('initr', 'nunif'))

    def add_param(self, param_id, shape, post=None, value=None):
        if value is None:
            if param_id == 'E':
                value = Parameter(shape, self.inite, name=param_id, post=post)
            elif param_id == 'R':
                value = Parameter(shape, self.initr, name=param_id, post=post)
            else:
...
class StochasticTrainer(object):
    ...
    def _batch_step(self, grads):
        for paramID in self._updaters.keys():
            if len(grads[paramID][0]) > 0: # If there are some idxs to update...
